...
int main(int argc, char *argv[])
{
    ...
    if (use_backend == TCP) {
        ...
        ctx = modbus_new_rtu("/dev/pts/6", 19200, 'E', 8, 1);
    }
    if (ctx == NULL) {
    ...
    */
    ...
    rc = modbus_read_bits(ctx, UT_BITS_ADDRESS, UT_BITS_NB
      , tab_rp_bits);
    printf("2/2 modbus_read_bits: ");
    if (rc != UT_BITS_NB) {
        printf("FAILED (nb points %d)\n", rc);
    ...
    printf("rc = %d,MODBUS_BROADCAST_ADDRESS = %d\n",rc,MODBUS_BROADCAST_ADDRESS);
